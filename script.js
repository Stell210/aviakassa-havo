...
document.querySelectorAll(".trip-btn").forEach(b=>b.classList.remove("active"));btn.classList.add("active");
...
const dep=document.getElementById("date"); if(dep) dep.min=todayISO;
const ret=document.getElementById("returnDate"); if(ret) ret.min=todayISO;
if(dep&&ret){dep.addEventListener("change",()=>{ret.min=dep.value||todayISO;if(ret.value&&ret.value<ret.min)ret.value=ret.min;});}
