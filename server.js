...
function isStandaloneGreetingText(text){
  const t=String(text||'').toLowerCase().replace(/[ё]/g,'е').trim();
  return /^(?:салом(?:\s+(?:алек|алекум|алейкум|алайкум))?|ассалом(?:\s+(?:алекум|алейкум|алайкум))?|ассалому\s+(?:алекум|алейкум|алайкум)|ваалейкум(?:\s+(?:ассалом|асалом))?|привет|здравствуйте|добрый\s+(?:день|вечер|утро)|hello|hi|hey)[!.,\s]*$/i.test(t);
}
...
async function aiAnalyze(instagramUserId,text){
  ...
  const standaloneGreeting=isStandaloneGreetingText(text);
  ...
  if(standaloneGreeting){
    // Reciprocal greeting: greetings never enter the flight flow or generic AI fallback.
    const greetingReply=detectedLanguage==='tj'
      ? 'Ваалайкум ассалом! 👋 Хуш омадед ба Aviakassa_havo. Чӣ гуна метавонам ба шумо кӯмак кунам?'
      : detectedLanguage==='en'
        ? 'Wa alaykum assalam! 👋 Welcome to Aviakassa_havo. How can I help you?'
        : 'Ваалейкум ассалом! 👋 Добро пожаловать в Aviakassa_havo. Чем могу помочь?';
    return {language:detectedLanguage,intent:'general',reply:greetingReply,name:'',phone:'',from_city:'',to_city:'',departure_date:'',return_date:'',trip_type:'',passengers:'',baggage:'',handoff:false,manager_waiting:false};
  }
