...
async function api(req,res,url){
  ...
  if(req.method==="GET" && url.pathname==="/api/health") return send(res,200,{ok:true,database:!!pool,adminPasswordConfigured:!!ADMIN_PASSWORD,whiteLabelId:TRAVELPAYOUTS_WHITE_LABEL_ID});
  if(req.method==="GET" && url.pathname==="/api/admin/config-status") return send(res,200,{ok:true,database:!!pool,adminPasswordConfigured:!!ADMIN_PASSWORD});
